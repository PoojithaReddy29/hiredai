...
from ai_logic import get_groq_response, extract_text_from_pdf, interview_chatbot, speech_to_text, text_to_speech

# Streamlit App Configuration helpers
...
def run():
    ...
    with tab1:
        ...
        if analyze_resume and uploaded_file:
            ...
            response = get_groq_response(input_text, resume_text, prompts["evaluation"])
            st.subheader("📌 Response:")
            st.write(response)
        elif check_match and uploaded_file:
            resume_text = extract_text_from_pdf(uploaded_file)
            response = get_groq_response(input_text, resume_text, prompts["percentage_match"])
    ...
    with tab2:
        ...
        if st.button("Start Interview"):
            if job_type:
                while True:
                    ai_question = get_groq_response(job_type, "", "Act as an interviewer. Ask questions one by one and provide feedback.")
                    ...
                    ai_feedback = get_groq_response(job_type, user_response, "Evaluate response, provide feedback and a score.")  
